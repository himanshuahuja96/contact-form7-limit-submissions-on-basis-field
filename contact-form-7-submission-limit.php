...
Version: 0.1
Plugin URI: https://github.com/himanshuahuja96/contact-form7-submission-limit
Description: Allows limiting submission on each form with unique error message
Author: Himanshu Ahuja
Author URI: https://www.webilabs.com
License: GPLv2 or later
Text Domain: contact-form7-submission-limit
 */

/*
Copyright 2019 Himanshu Ahuja
...
$i = 0;
    ...
    if (isset($cf7sl_settings['cf7slformid']) && is_array($cf7sl_settings['cf7slformid'])) {
        ...
        foreach ($cf7sl_settings['cf7slformid'] as $key => $find) {
            ...
            if(isset($cf7sl_settings['cf7slinterval'][$key])){
                $cf7sl_interval = $cf7sl_settings['cf7slinterval'][$key];
            }
            else{
                $cf7sl_interval = '-1';
            }
            ...
            echo "<div style='float: left'>";
            echo "<label for='interval$i'>Submission Interval:</label>";
            echo "<br />";
            echo "<select name='cf7slinterval[$i]' id='cf7slinterval$i'>";
            $intervals = array(
                'INTERVAL 1 DAY',
                'INTERVAL 7 DAY',
                'INTERVAL 30 DAY',
            ...
            if ($intervals) {
                foreach ($intervals as $key ) {
                    if ($key == $cf7sl_interval  ) {
                        echo "<option  name='cf7slformid[$i]' id='cf7slformid$i' value='" . $key. "' selected>" . $key . "</option>";
                    } else {
                        echo "<option  name='cf7slformid[$i]' id='cf7slformid$i' value='" . $key. "'>" . $key. "</option>";
                    }
                }
            }
            echo "</select>";
            ...
            echo "<div style='float: left'>";
            echo "<div id='status$i' name='status$i' style='display:none'>" . $status . "</div>";
            echo "</div>";
            echo "<br />";
            echo "</div>";
    ...
    <div id="cf7sl-sb">
        <div class="postbox" id="cf7sl-sbone">
            ...
            <div class="inside">
                ...
                <li>Enter your limit (if the number are above or equal the total number of submissions then the user won't be able to submit).</li>
                <li>Enter any HTML code or the message that you want to show to user when they try to over submit.</li>
        ...
        <div class="postbox" id="cf7sl-sbtwo">
            ...
            <div class="inside">
                <p>Your best bet is to post on the <a href="https://github.com/himanshuahuja96/">plugin support page</a>.</p>
                <p>Please consider supporting me by <a href="https://github.com/himanshuahuja96/">rating this plugin</a>. Thanks!</p>
            </div>
        </div>
    </div>
...
{
    wp_enqueue_script('cf7sl_dynamicfields', plugin_dir_url(__FILE__) . 'js/jquery.dynamicfields.js', array('jquery'));
    wp_enqueue_script('jquery-ui-1', plugin_dir_url(__FILE__) . 'js/jquery-ui-1.10.3.custom.min.js', array('jquery'));
    //wp_enqueue_script('cf7sl_functions', plugin_dir_url(__FILE__) . 'js/cf7sl.functions.js', array('jquery'));
    wp_enqueue_style('cf7sl_styles', plugin_dir_url(__FILE__). 'css/cf7sl.css');
}
...
function is_submition_above_limit($form_id, $limit)
{
    ...
    //$result = $wpdb->get_results('SELECT form_name,count(DISTINCT `submit_time`) as "count" FROM ' . $wpdb->prefix . 'cfdb7_database WHERE form_name = \'' . $form_name . '\' ');
    
    $cf7sl_settings = get_option('cf7sl_plugin_settings');
    $indexAt = checkInlist( $cf7sl_settings['cf7slformid'],$form_id );
    if ($indexAt && is_array($cf7sl_settings['cf7slinterval'])) {
        $intervalVal = $cf7sl_settings['cf7slinterval'][$indexAt];
    }
    
    if($intervalVal == NULL || $intervalVal == '' || $intervalVal == false)
        $intervalVal = 'INTERVAL 1 DAY';
    else if($intervalVal === 'NEVER AGAIN'){
        $intervalVal = 'INTERVAL 1000 YEAR';
    }
        
    $result = $wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'db7_forms WHERE form_post_id = ' .$form_id.' AND form_date > DATE_SUB(NOW(),'     . $intervalVal. ')',OBJECT);
    
    if($result == NULL || $result == '' || $result == false){
        return false;
    }
    
    // error_log($intervalVal);
     error_log($wpdb->last_query);
    
    $form_value = array_map(function($e) {
    return is_object($e) ? $e->form_value : $e['form_value'];
    }, $result);
    ...
    if ($form_value) {
        foreach( $form_value as $value ){
             ...
             if(array_search( get_client_ip() , $unserializeData )){
                 $count++;
            }
        }
        //error_log("count".$count);
        //error_log("condition ".$count >= $limit ? true : false);
...
function get_total_submitions()
{
    ...
    $total_submitions = $wpdb->get_results('SELECT form_post_id, count(*) as "count"
FROM ' . $wpdb->prefix . 'db7_forms GROUP BY form_post_id',ARRAY_A );
    if ($total_submitions) {
...
add_filter( 'wpcf7_spam', 'custom_email_confirmation_validation_filter', 20, 2 );
 
function custom_email_confirmation_validation_filter( $spam ) {
    $currentForm = $_POST['_wpcf7'] != null && $_POST['_wpcf7'] != '' ? $_POST['_wpcf7'] : -1 ;
    $cf7sl_settings = get_option('cf7sl_plugin_settings');
    if (is_array($cf7sl_settings['cf7slformid']) && $currentForm) {
        $indexAt = checkInlist( $cf7sl_settings['cf7slformid'],$currentForm );
        //error_log("IndexAt".$indexAt);
        if($indexAt){
            //error_log("is_submition_above_limit".is_submition_above_limit($currentForm,$cf7sl_settings['cf7sllimit'][$indexAt]));
            //error_log("limit:".$cf7sl_settings['cf7sllimit'][$indexAt]);
            if(is_submition_above_limit($currentForm,$cf7sl_settings['cf7sllimit'][$indexAt])){
                // $result->invalidate(
                //     $tag,
                //     $cf7sl_settings['cf7slreplace'][$indexAt]
                // );  
                $spam = true;
            }
        }
        ...
        $cf7sl_settings = get_option('cf7sl_plugin_settings');
            if (is_array($cf7sl_settings['cf7slformid']) && $currentForm) {
                $indexAt = checkInlist( $cf7sl_settings['cf7slformid'],$currentForm );
                    //error_log("IndexAt".$indexAt);
                    if($indexAt){
    			        $message = $cf7sl_settings['cf7slreplace'][$indexAt];
                    }
            }
			else{
			    $message = __( 'Trying to do Spam? Huh!', '');
			}
		}
return $message;
}
...
function checkInlist( $formsIdArray,$toCheck){
    //error_log("CheckINList:" .array_search($toCheck ,$formsIdArray ));
    return array_search($toCheck ,$formsIdArray );
}
...
function get_submitted_at(){
    return time();
}
add_shortcode("get_submitted_at","get_submitted_at");
